...
# Allowed constructor args to avoid passing invalid kwargs to sklearn
ALLOWED_ARGS = {
    "LogisticRegression": {"C", "penalty", "solver", "max_iter"},
    "SVM": {"C", "kernel", "gamma", "degree", "probability"},
    "DecisionTree": {"max_depth", "min_samples_leaf"},
    "RandomForest": {"n_estimators", "max_depth", "max_features"},
    "KNN": {"n_neighbors", "weights"},
    "NeuralNet": {"hidden_layer_sizes", "activation", "learning_rate_init", "batch_size", "max_iter"}
}
...
DEFAULT_DATASETS = {
    ...
    "Blobs": lambda: datasets.make_blobs(n_samples=300, centers=3, random_state=0),
    "Moons": lambda: datasets.make_moons(n_samples=300, noise=0.2, random_state=0)
}

def normalize_val(v):
    # convert numpy types and numeric strings to Python primitives; "None"->None
    if v is None:
        return None
...
def filter_hyperparams(model_name, hyperparams):
    allowed = ALLOWED_ARGS.get(model_name, set())
    out = {}
    for k, v in (hyperparams or {}).items():
        if k in allowed:
            out[k] = normalize_val(v)
    return out

def load_dataset(name_or_csv):
    if isinstance(name_or_csv, dict) and 'csv' in name_or_csv:
        from io import StringIO
        df = pd.read_csv(StringIO(name_or_csv['csv']))
        X = df.iloc[:, :-1].values
        y = df.iloc[:, -1].values
        feature_names = list(df.columns[:-1])
        return X, y, feature_names
    if name_or_csv in DEFAULT_DATASETS:
        out = DEFAULT_DATASETS[name_or_csv]()
        if isinstance(out, tuple) and len(out) == 2 and isinstance(out[0], np.ndarray):
            X, y = out
            feature_names = [f"f{i}" for i in range(X.shape[1])]
            return X, y, feature_names
        else:
            return out.data, out.target, list(out.feature_names)
    raise ValueError("Unknown dataset")

def build_model_safe(model_name, hyperparams):
    cls = MODEL_MAP.get(model_name)
    if not cls:
        raise ValueError("Unsupported model")
    safe_params = filter_hyperparams(model_name, hyperparams)
    # compatibility: penalty='none' for LogisticRegression often requires solver='saga'
    if model_name == "LogisticRegression":
        if safe_params.get("penalty") == "none":
            if safe_params.get("solver") not in ("saga", "lbfgs", "newton-cg", "sag"):
                safe_params["solver"] = "saga"
